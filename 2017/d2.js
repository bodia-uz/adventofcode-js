const spreadsheet = parseSpreadsheet(`
...
getCheckSum(spreadsheet, getMinMaxCheckSum)/*?*/;
...
getCheckSum([
  ...
  [9, 4, 7, 3],
  [3, 8, 6, 5]
], getEvenlyDivisibleCheckSum);/*?*/
getCheckSum(spreadsheet, getEvenlyDivisibleCheckSum)/*?*/;
...
function parseSpreadsheet(spreadsheetString) {
  return spreadsheetString.trim().split('\n').map(rowString => rowString.split(/\s/))
}
